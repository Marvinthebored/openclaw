...
suite.define(() => {
  it("persists height drags and routes width drags through the Message width setting", async () => {
    if (!instance) {
    ...
    const chatUrl = new URL(controlUiSessionUrl(suite.server.baseUrl, sessionKey, "chat"));
    ...
    const artifactDir = createControlUiE2eArtifactDir("composer-resize");
