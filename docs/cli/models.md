...
- The `Auth` column uses read-only checks. For provider-owned OpenAI routes, it matches each API and base URL to eligible profiles, credentials, and resolved command-scoped SecretRefs. If route policy is unavailable, an OpenAI row stays unknown instead of using provider-level auth. Other providers and legacy checks keep provider-level behavior. For a configured native CLI route, the command can run a provider-owned local auth-status check. Synthetic-auth metadata alone is not proof of native account authentication. The command does not load the full provider runtime, read keychain secrets, call provider APIs, or prove exact execution readiness.
