...
For example, a button with value `/model deepseek/deepseek-chat -s` can send that value as an encrypted Matrix message. The flag makes session scope explicit.
